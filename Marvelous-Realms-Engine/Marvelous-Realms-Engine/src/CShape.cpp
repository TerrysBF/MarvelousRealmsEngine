...
CShape::createShape(ShapeType type) {
	...
	case ShapeType::RECTANGLE: {
		auto rectSP = EngineUtilities::MakeShared<sf::RectangleShape>(sf::Vector2f(1.f, 1.f));
...
CShape::setTexture(const EngineUtilities::TSharedPointer<Texture>& texture) {
	if (!m_shapePtr || texture.isNull()) return;

	sf::Texture& tex = texture->getTexture();
	m_shapePtr->setTexture(&tex, true);

	if (m_shapeType == ShapeType::RECTANGLE) {
		auto rect = m_shapePtr.dynamic_pointer_cast<sf::RectangleShape>();
		if (!rect.isNull()) {
			const sf::Vector2u sz = tex.getSize();
			rect->setSize(sf::Vector2f(static_cast<float>(sz.x), static_cast<float>(sz.y)));
			sf::IntRect fullRect(
				sf::Vector2i(0, 0),
				sf::Vector2i(static_cast<int>(sz.x), static_cast<int>(sz.y))
			);
			rect->setTextureRect(fullRect);
